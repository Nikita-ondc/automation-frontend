...
Describes the type of MIME. It can be one of the three possible values: `application/html` or `text/html` or `text/html-multi`.
...
- **text/html**:
    ...
    - **Initiate a Fetch Request**: The Buyer App must send an HTTP fetch request to the URL provided by the Seller App. This URL is included in the `xinput` object as part of on_action calls.

    - **Process the Response**: When the Seller App receives this request, it will respond with the complete HTML document.

    - **Render the HTML Document**: Once the complete HTML document is retrieved, the Buyer App must render it on the user interface as intended. This could involve displaying it within a web view or a similar component in the app, ensuring that the user has a seamless and interactive experience.
    ...
    Note: While submitting a `text/html` form, API headers will contain `Content-Type: "multipart/form-data"`.
...
1. #### Form response for text/html
    ...
    On successful submission the buyer is provided with a `submission_id`, which the buyer can use to send as part of the next call.
...
2. #### Form response for application/html
    ...
    To provide the buyer with the latest form status, the seller sends an unsolicited on_status call with the `Submission_ID`. Additionally, if the seller doesn't send an unsolicited call, the buyer can request the latest form status by sending a status call with the `ref_id`.
    ...
    - The seller responds with the latest status of the submitted form corresponding to the `submission_id` ("Submission_ID") that was submitted earlier by the buyer.
